...
void receive_poll(PtTimestamp timestamp, void *userData)
{
    ...
    while ((count = Pm_Read(global_pPmStreamMIDIIN, &event, 1))) 
	{
        if (count == 1) 
		{
			...
			{
				if(global_notenumber>-1 && global_notenumber<128)
				{
					/*
					//send note off
					PmEvent tempPmEvent;
					tempPmEvent.timestamp = 0;
					tempPmEvent.message = Pm_Message(0x90+outputmidichannel, global_notenumber, 0); //note off, channel 0
					Pm_Write(global_pPmStreamMIDIOUT, &tempPmEvent, 1);
					*/
				}
				global_notenumber = Pm_MessageData1(event.message);
			}
...
int main(int argc, char **argv)
{
    ...
    */
	...
	cout << "device " << outputmididevice << " selected" << endl;
    //err = Pm_OpenInput(&midi_in, inp, NULL, 512, NULL, NULL);
    err = Pm_OpenOutput(&global_pPmStreamMIDIOUT, outputmididevice, NULL, 512, NULL, NULL, 0); //0 latency
    if (err) 
	{
    ...
    err = Pm_OpenInput(&global_pPmStreamMIDIIN, inputmididevice, NULL, 512, NULL, NULL);
    if (err) 
	{
        printf(Pm_GetErrorText(err));
        Pt_Stop();
	...
	*/
	...
	while (GetMessage(&Msg, NULL, 0, 0)) 
	{
		...
		{
			/*
			Pm_Write(global_pPmStreamMIDIOUT, &myPmEvent[1], 1);
			Pm_Write(global_pPmStreamMIDIOUT, &myPmEvent[0], 1);
			*/
			if(global_notenumber>-1 && global_notenumber<128)
			{
				tempPmEvent.timestamp = 0;
				...
				Pm_Write(global_pPmStreamMIDIOUT, &tempPmEvent, 1);
				tempPmEvent.timestamp = 0;
				tempPmEvent.message = Pm_Message(0x90+outputmidichannel, global_notenumber, 100); //note on, channel 0
				Pm_Write(global_pPmStreamMIDIOUT, &tempPmEvent, 1);
			}
		}
		else
